...
def whoami(request):
    if request.POST['token']:
        this_token = request.POST.get('token')  # TODO: Check if there is no `token`- done-please Check it
...
def submit_expense(request):
    ...
    # TODO: revise validation for the amount
    this_date = request.POST.get('date') if 'date' in request.POST else timezone.now()
    this_text = request.POST.get('text') if 'text' in request.POST else ""
    this_amount = request.POST.get('amount') if 'amount' in request.POST else "0"
    this_token = request.POST.get('token') if 'token' in request.POST else ""
    this_user = get_object_or_404(User, token__token=this_token)
